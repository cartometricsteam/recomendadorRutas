(function () { function r(e, n, t) { function o(i, f) { if (!n[i]) { if (!e[i]) { var c = "function" == typeof require && require; if (!f && c) return c(i, !0); if (u) return u(i, !0); var a = new Error("Cannot find module '" + i + "'"); throw a.code = "MODULE_NOT_FOUND", a } var p = n[i] = { exports: {} }; e[i][0].call(p.exports, function (r) { var n = e[i][1][r]; return o(n || r) }, p, p.exports, r, e, n, t) } return n[i].exports } for (var u = "function" == typeof require && require, i = 0; i < t.length; i++)o(t[i]); return o } return r })()({
    1: [function (require, module, exports) {
    ...
    }, { "ieee754": 4 }], 9: [function (require, module, exports) {
        'use strict'
        ...
        if (!mapboxgl.supported()) {
            alert('Your browser does not support Mapbox GL. This app works on Safari 9 and above, Microsoft Edge 13 and above, along with the latest version of Chrome and Firefox');
        }
        else {
            /**
            * Add our API token
            ...
            */
            const url = 'assets/data/trails.min.pbf',
                center_point = [          -4.978179931640624,
                    36.691547435472636],
                bounds = [[          -5.2027130126953125,
                    36.54108783626331], [          -4.8099517822265625,
                        36.84336173438382]],
            ...
            const start_input = document.getElementById('map__start'),
                end_input = document.getElementById('map__end'),
                time_input = document.getElementById('map__time'),
                menu = document.getElementById('map__menu'),
                overlay = document.getElementById('map__overlay'),
                toggle = document.getElementById('map__toggle'),
                difficulty = document.getElementById('map__difficulty'),
                distance = document.getElementById('map__distance'),
                ramp = document.getElementById('map__ramp'),
                duration = document.getElementById('map__duration');
            ...
            map.on('load', () => {
                fetch(new Request(url)).then(response => {
                    ...
                    data = geobuf.decode(new Pbf(response));

                    /**
                    * Add options to the select elements
                    */
                    let start_set = new Set(),
                        end_set = new Set();
                    data['features'].forEach(feature => {
                        start_set.add(feature['properties']['start']);
                        end_set.add(feature['properties']['end']);
                    });
                    start_set.forEach(location => {
                        let option = document.createElement('option');
                        option.setAttribute('value', location);
                        option.appendChild(document.createTextNode(location))
                        start_input.appendChild(option)
                    });
                    end_set.forEach(location => {
                        let option = document.createElement('option');
                        option.setAttribute('value', location);
                        option.appendChild(document.createTextNode(location))
                        end_input.appendChild(option)
                    });
                    map.addSource('dem', {
                        'type': 'raster-dem',
            ...
            toggle.addEventListener('click', () => {
                ...
                menu.classList.toggle('map__menu--visible');
                overlay.classList.toggle('map__overlay--expanded')
            });

            start_input.addEventListener('change', e => {
                let layer_filter = map.getFilter('trails'),
                    revised_filter = layer_filter.filter(filter => filter.includes('all') != true),
                    new_filter = layer_filter.filter(filter => filter.includes('all'))[0]
                        .filter(filter => filter[1].includes('start') === false)
                        .concat([['==', ['get', 'start'], e['target']['value']]]);
                map.setFilter('trails', revised_filter.concat([new_filter]));
            });

            end_input.addEventListener('change', e => {
                let layer_filter = map.getFilter('trails');
                if (e['target']['value'] != '') {
                    let revised_filter = layer_filter.filter(filter => filter.includes('all') != true),
                        new_filter = layer_filter
                            .filter(filter => filter.includes('all'))[0]
                            .filter(filter => filter[1].includes('end') === false)
                            .concat([['==', ['get', 'end'], e['target']['value']]]);
                    map.setFilter('trails', revised_filter.concat([new_filter]));
            ...
            time_input.addEventListener('input', e => {
                let layer_filter = map.getFilter('trails'),
                    revised_filter = layer_filter.filter(filter => filter.includes('all') != true),
                    new_filter = layer_filter.filter(filter => filter.includes('all'))[0]
                        .filter(filter => filter[0].includes('<=') === false)
                        .concat([['<=', ['to-number', ['get', 'time']], parseFloat(e['target']['value']) * 3600]]);
                map.setFilter('trails', revised_filter.concat([new_filter]));
            });

            [start_input, time_input, end_input].forEach(element => {
                element.addEventListener('change', () => {
                    if (start_input.value != '' && end_input.value != '') {
                        let path = data['features']
                            .filter(trail => (trail['properties']['start'] === start_input.value && trail['properties']['end'] === end_input.value))[0]
                        if (path != undefined && (path['properties']['time'] / 3600 < time_input.value || time_input.value === '')) {
                            difficulty.textContent = (path['properties']['difficulty'] < 2 ? 'Baja' : (path['properties']['difficulty'] > 4 ? 'Alta' : (path['properties']['difficulty'] < 3 ? 'Media-Baja' : (path['properties']['difficulty'] > 3 ? 'Media-Alta' : 'Media'))));
                            distance.textContent = path['properties']['route_length'];
                            ramp.textContent = path['properties']['ramp_up'] + '↗ | ' + path['properties']['ramp_down'] + '↘'
                            duration.textContent = moment.duration(path['properties']['time'], 'seconds').humanize();
                        }
                        else {
    ...
    }, { "geobuf": 3, "mapbox-gl": 5, "moment": 7, "moment/locale/es": 6, "pbf": 8 }]
}, {}, [9]);
